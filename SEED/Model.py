...
class Model(nn.Module):
    def __init__(self, xdim, kadj, num_out, dropout):
        ...
        self.lstm = nn.LSTM(62, 9, 9, bidirectional=True)

        self.A = nn.Parameter(torch.FloatTensor(4).cuda())

        self.fc62 = nn.Sequential(
            # nn.BatchNorm1d(248),
            nn.Linear(channel_num * channel_num, channel_num),
            nn.BatchNorm1d(channel_num),
            nn.Dropout(0.5),
            nn.Sigmoid(),
        )

        self.fc3 = nn.Linear(channel_num, 3)
        self.fc4 = nn.Linear(channel_num, 4)


    def forward(self, de):
        # l = torch.tensor([[0.298, 0.396, 0.306], [0.325,0.530,0.145], [0.282,0.569,0.150], [0.281,0.719,0]], requires_grad=True).cuda()
        l = torch.tensor(np.load("G:/吴老师的论文/规则集解释/SEED/15_20131105_rules.npy"), requires_grad=True).cuda()
        ...
        # print(output.shape)
        rule = F.softmax(output1, dim=1)
        rule = self.A * rule
        r = torch.argmax(rule, dim=1)
        # result = l[torch.arange(l.size(0)), r]
        result = l[r.squeeze(-1)]

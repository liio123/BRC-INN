...
#9-6
# DE = torch.tensor(np.load("G:/组合脑相关内容/SEED/第一个会话的DE/1.npy").real.astype(float), dtype=torch.float)
# rule = np.load("G:/吴老师的论文/规则集解释/SEED/1_20131027_rules.npy")
# print(rule)
#
...
# #                           np.ones([206, 1])])
# train_DE = DE[0:2010, :, :]
# test_DE = DE[2010:3394, :, :]
# train_label = np.concatenate([np.full((235,1), 2), np.ones([233, 1]), np.zeros([206, 1]), np.zeros([238, 1]), np.ones([185, 1]), np.full((195,1), 2), np.zeros([237, 1]),
#                           np.ones([216, 1]), np.full((265,1), 2)])
# test_label = np.concatenate([np.full((237,1), 2), np.ones([235, 1]), np.zeros([233, 1]), np.ones([235, 1]), np.full((238,1), 2),
#                           np.ones([206, 1])])
...
#
# train_data = TensorDataset(train_DE, train_label)
# test_data = TensorDataset(test_DE, test_label)
...
# # opt = torch.optim.SGD(model.parameters(), lr=learning_rate, momentum=0.5)
#
# train_acc_list = []
# train_loss_list = []
# test_acc_list = []
...
#
# G = testclass()
# train_len = G.len(train_DE.shape[0], batch_size)
# test_len = G.len(test_DE.shape[0], batch_size)
#
...
#五折交叉验证
DE = torch.tensor(np.load("G:/组合脑相关内容/SEED-IV/test/DE/3/15.npy").real.astype(float), dtype=torch.float)
# rule = np.load("G:/吴老师的论文/规则集解释/SEED/1_20131027_rules.npy")
#会话一
...
for train_idx, test_idx in kfold.split(MyDataset):
    ...
    # -------------------------------------------------
        ...
        if (total_test_acc / test_len) > min_acc:
            ...
            torch.save(model.state_dict(), 'G:/吴老师的论文/规则集解释/模型可视化/SEED-IV-3-15.pth')
        # print result
        print("Epoch: {}/{} ".format(epoch + 1, num_epochs),
              "Training Loss: {:.4f} ".format(total_train_loss / len(train_loader)),
              "Training Accuracy: {:.4f} ".format(total_train_acc / train_len),
